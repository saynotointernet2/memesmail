...
(
  ...
  CONSTRAINT mm_user_key PRIMARY KEY (user_id)
)
WITH (
  OIDS=FALSE
);
...
(
  object_id mm_object_id NOT NULL DEFAULT encode(decode(replace(((gen_random_uuid())::character varying)::text, '-'::text, ''::text), 'hex'::text), 'base64'::text),
  user_id mm_user_id,
  body mm_object_body,
  CONSTRAINT mm_object_key PRIMARY KEY (object_id),
...
(
  key_id mm_object_key_id NOT NULL DEFAULT encode(decode(replace(((gen_random_uuid())::character varying)::text, '-'::text, ''::text), 'hex'::text), 'base64'::text),
