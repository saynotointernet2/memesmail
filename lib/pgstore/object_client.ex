defmodule Memesmail.Pgstore.ObjectClient do
  ...
  def store_object(user_id, object_body, keys) do
    ...
    |> (&Postgrex.execute!(Server.server_name, &1, [user_id, object_body, keys]).rows).()
    |> hd
    |> hd
    |> (&{:ok, &1}).()
  end
  ...
  def add_keys(user_id, object_id, keys) do
    ...
    |> (&Postgrex.execute!(Server.server_name, &1, [user_id, object_id, keys]).rows).()
    |> hd
    |> hd
    |> (&{:ok, &1}).()
  end
  ...
  def edit_object(user_id, object_id, object_body, keys) do
    ...
    |> (&Postgrex.execute!(Server.server_name, &1, [user_id, object_id, object_body, keys]).rows).()
    |> hd
    |> hd
    |> (&{:ok, &1}).()
  end
  ...
  def remove_keys(user_id, object_id, key_ids) do
    ...
    |> (&Postgrex.execute(Server.server_name, &1, [user_id, object_id, key_ids])).()
  end
